...
- **Kiểm tra Đường dẫn & Thư mục (Strict Path & Folder Verification)**: Tuyệt đối KHÔNG sinh mã nguồn sai thư mục (VD: file giao diện phải nằm trong `views/`, không được vứt vào `utils/` hay thư mục gốc). Khi tạo mới file, AI bắt buộc phải đối chiếu với cây thư mục trong `PLAN.md`, xuất mã `diff` với đường dẫn tuyệt đối chính xác (`+++ /đúng/thư/mục/file.py`), và **phải có lời nhắc người dùng tạo thư mục đó trước** nếu nó chưa tồn tại.
