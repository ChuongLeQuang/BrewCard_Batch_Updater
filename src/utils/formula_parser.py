...
class ExcelFormulaEvaluator:
    ...
    def evaluate(self, formula: str) -> Any:
        ...
        try:
            ...
            self.errors.append(f"Công thức '{formula}': {e}")
            return "#DIV/0!"
            ...
            self.errors.append(
                f"Công thức '{formula}': Lỗi cú pháp hoặc tham chiếu không hợp lệ."
            )
            return "#ERROR!"
    ...
    def _to_num(self, val: Any) -> float:
        ...
        try:
            ...
            raise TypeError(f"Không thể chuyển đổi '{val}' thành số.")
    ...
    def _eval_node(self, node: Any) -> Any:
        ...
        if kind == "LITERAL":
            ...
            val = self._eval_node(node[1])
            return self._to_num(val) / 100.0
        elif kind == "UNARY":
            ...
            if op in ("+", "-", "*", "/", "^"):
                ...
                if op == "/":
                    if r_num == 0.0:
                        raise ZeroDivisionError(f"Lỗi chia cho 0 (mẫu số là '{r_val}')")
                    return l_num / r_num
                if op == "^":
                    return l_num**r_num
            elif op in ("=", "<>", ">", "<", ">=", "<="):
    ...
    def _parse_percentage(self) -> Any:
