...
1. **Cấu hình (Config)**: Người dùng nhập thông số, click chọn tọa độ ô mẫu hoặc **Nạp File Công thức Tự động (Smart Auto-Import)** -> Dữ liệu lưu vào `data/configuration.json`.
...
### 2.5. Các rủi ro & Nợ kỹ thuật (Đã nhận diện)
1. **Lỗi tính toán với toán tử `%`**: Cây AST chưa hỗ trợ toán tử `%` (chia 100) theo tư duy hậu tố (Postfix). Sẽ gây sai lệch hoặc lỗi `#ERROR!` nếu khách hàng nạp công thức gốc (Ưu tiên Cao nhất).
2. **Vi phạm kiến trúc phân tầng**: Tính năng `Smart Auto-Import` trong `config_widget_mappings.py` đang nhồi nhét quá nhiều logic nghiệp vụ (đọc file, chuẩn hóa, so khớp AI) vào lớp View, làm phình to file và không thể viết Unit Test.
3. **Hardcode Tiếng Việt**: Các từ khóa nhận diện cột (tên, công thức, đích, nguồn) bị ghi cứng trong code giao diện, vi phạm nguyên tắc Clean Code và quy định 100% English.
4. **Thiếu Unit Test**: Lớp dữ liệu trung tâm `AppConfig` và các logic phụ trợ chưa được bảo vệ bởi Test tự động.
...
- `src/views/config_widget_mappings.py`: Khung quản lý Lưới Mapping dữ liệu và Tính năng Nạp tự động từ File (Smart Auto-Import với Fuzzy Matching).
- `src/views/config_dialog_alias.py`: Hộp thoại Cảnh báo ghép nối thủ công (Human-in-the-loop) cho các cột sai tên.
...
| 14. Nạp tự động công thức từ file (Smart Import & Merge) | `src/views/config_widget_mappings.py` |
| 15. Hộp thoại Cảnh báo Ghép nối thủ công (Alias Mapping) | `src/views/config_dialog_alias.py` |
...
- [x] **Bước 7 (Enhancement)**: Thêm tính năng "Nạp File Công thức Tự động", giải quyết triệt để rủi ro nhập liệu tay từ file khách hàng.
- [x] **Bước 8 (Enhancement - Human-in-the-loop)**: Xây dựng Hộp thoại Cảnh báo ghép nối (Alias Mapping Dialog) để xử lý các cột khách hàng đặt sai tên khi nạp file.

**Giai đoạn 3: Trả nợ Kỹ thuật & Chuẩn hóa (Refactoring & Stabilization)**
*   **Bước 9: Vá lỗi Cốt lõi Toán học (AST % Operator)**
    - [x] **9.1 (Test)**: Cập nhật `tests/test_formula_parser.py` để thêm các kịch bản test cho toán tử `%` (TDD).
    - [x] **9.2 (Code)**: Nâng cấp `src/utils/formula_parser.py` để xử lý đúng logic và độ ưu tiên của toán tử `%`.
    - [x] **9.3 (Verify)**: Chạy `auto_checks.py` để xác nhận toàn bộ test đã pass.

*   **Bước 10: Tái cấu trúc & Dọn dẹp (Refactoring & Clean Code)**
    - [x] **10.1 (Constants)**: Gom các hằng số Tiếng Việt (tên, cột, công thức...) vào `src/config/constants.py`.
    - [x] **10.2 (Refactor-View)**: Cập nhật `config_widget_mappings.py` để sử dụng hằng số từ `constants.py`.
    - [x] **10.3 (Service)**: Gom logic trích xuất Excel vào `src/services/excel_reader_service.py`.
    - [x] **10.4 (Integration)**: Gọt lại `config_widget_mappings.py` để nó chỉ còn vai trò gọi Service và hiển thị kết quả.
    - [x] **10.5 (Verify)**: Chạy `auto_checks.py` để xác nhận tính năng vẫn hoạt động sau tái cấu trúc.

*   **Bước 11: Tăng cường Vùng phủ Kiểm thử (Test Coverage)**
    - [x] **11.1 (Test)**: Viết Unit Test cho `src/models/config_model.py` (lưu, tải, xóa profile).
    - [x] **11.2 (Test)**: Viết Unit Test cho `src/services/excel_reader_service.py` (Kiểm thử logic trích xuất, chuẩn hóa, phân loại nạp tự động).
    - [x] **11.3 (Verify)**: Chạy `auto_checks.py` lần cuối để nghiệm thu toàn bộ Giai đoạn 3.

**Giai đoạn 4: Tiện ích Nâng cao & Trải nghiệm (Advanced Features)**
*   **Bước 12: Quản lý Định dạng Hiển thị (OpenPyXL Number Format)**
    - [x] **12.1 (Constants)**: Định nghĩa từ điển các định dạng chuẩn (Text, Int, Float, %, Date, Time) trong `src/config/constants.py` để đồng bộ giữa UI và Code Excel.
    - [x] **12.2 (UI/View)**: Bổ sung cột thứ 4 "Định dạng hiển thị" (QComboBox) vào lưới Mappings trong `config_widget_mappings.py`. Cập nhật logic `get_mappings` và `load_mappings` để lưu xuống JSON.
    - [x] **12.3 (Smart UX)**: Thêm cơ chế tự động đoán định dạng khi nạp file (VD: công thức chứa ký tự `%` -> tự set ComboBox thành `%`).
    - [x] **12.4 (Service)**: Cập nhật `BrewSyncEngine` (`excel_sync_service.py`) để đọc cấu hình định dạng và áp dụng `cell.number_format` khi ghi dữ liệu.
    - [x] **12.5 (Test & Verify)**: Viết bổ sung Unit Test kiểm chứng định dạng và chạy `auto_checks.py` nghiệm thu.

**Giai đoạn 5: Gia cố Tính năng Nạp file (Hardening Smart Import)**
*   **Bước 13: Xây dựng Nền tảng Dữ liệu & Bộ lọc Lõi (Core & Memory)**
    - [x] **13.1 (Memory)**: Tạo `src/models/alias_memory.py` quản lý `data/alias_memory.json`.
    - [x] **13.2 (Sanitize)**: Viết hàm `sanitize_formula()` trong `excel_reader_service.py` lọc file rác.
    - [x] **13.3 (Validation)**: Sửa `extract_formulas_from_sheet` để chặn tên cột trùng lặp.
    - [x] **13.4 (Test)**: Viết Unit Test cho các tính năng lõi mới.
*   **Bước 14: Dựng Hộp thoại Phân công Cột (Giao diện Tiền xử lý)**
    - [x] **14.1 (Service)**: Thêm hàm `get_sheet_preview()` đọc 20 dòng đầu.
    - [x] **14.2 (UI)**: Dựng `dialog_column_selector.py` cho phép chọn cột/dòng.
    - [x] **14.3 (Integration)**: Bắt lỗi `ValueError` khi không tìm thấy tiêu đề và mở hộp thoại.
*   **Bước 15: Nâng cấp AI Gợi ý & Hộp thoại Cảnh báo (Giao diện Hậu xử lý)**
    - [x] **15.1 (AI)**: Cập nhật thuật toán tính điểm kép (Tên + Công thức) trong `AliasMappingDialog`.
    - [x] **15.2 (UI)**: Mở khóa cột Tên Khách gửi để cho phép sửa lỗi chính tả.
    - [x] **15.3 (Integration)**: Tích hợp đối chiếu `AliasMemory` trước khi mở hộp thoại.

**Giai đoạn 6: Kiểm thử Tích hợp Đầu cuối (E2E Sandbox Testing)**
*   **Bước 17: Xây dựng Kịch bản Sandbox E2E**
    - [x] **17.1 (Data Prep)**: Chuẩn bị 2 file đầu vào: `mock_input_clean.xlsx` (Dữ liệu hoàn hảo) và `mock_input_dirty.xlsx` (Cố tình để trống E10, F40...).
    - [x] **17.2 (Happy Path)**: Viết kịch bản `test_e2e_happy_path`, ép hệ thống chạy file `clean`, kiểm chứng KHÔNG có bất kỳ log lỗi nào, data và format phải hoàn hảo 100%.
...
- **20/05/2026**: Hoàn thành Bước 7 và Bước 8 (Nạp File Công Thức tự động & Hộp thoại Human-in-the-loop).
